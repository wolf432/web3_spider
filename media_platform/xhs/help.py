...
def chinese_to_number(chinese_str):
    ...
    if isinstance(chinese_str, (int, float)):  # 如果传入的是数字，则直接返回
        return chinese_str
    ...
    for char in chinese_str:
        if char.isdigit() or char == '.':
            ...
            if num_str:
                temp_num = float(num_str) if '.' in num_str else int(num_str)
            else:
                temp_num = 1  # 如果单位前面没有数字，默认是1
            result += temp_num * units[char]
            num_str = ''
